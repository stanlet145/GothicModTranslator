...
{
	kurwaAI_Output(self,other,"DIA_Harad_Hallo_12_00");	//Czego chcesz?
};
...
func void DIA_Harad_Arbeit_Info()
{
	...
	if((MIS_Jack_NewLighthouseOfficer != LOG_SUCCESS) && !Npc_IsDead(Brian))
	{
...
func int DIA_Harad_OrcSuccess_Condition()
{
	...
	{
		if(Npc_HasItems(other,ItMw_2H_OrcAxe_01) || Npc_HasItems(other,ItMw_2H_OrcAxe_02) || Npc_HasItems(other,ItMw_2H_OrcAxe_03) || Npc_HasItems(other,ItMw_2H_OrcAxe_04) || Npc_HasItems(other,ItMw_2H_OrcSword_02))
		{
			return TRUE;
		};
	};
...
func void DIA_Harad_OrcSuccess_Info()
{
	...
	else if(Hlp_IsItem(EquipWeap,ItMw_2H_OrcAxe_01) || Hlp_IsItem(EquipWeap,ItMw_2H_OrcAxe_02) || Hlp_IsItem(EquipWeap,ItMw_2H_OrcAxe_03) || Hlp_IsItem(EquipWeap,ItMw_2H_OrcAxe_04) || Hlp_IsItem(EquipWeap,ItMw_2H_OrcSword_02))
	{
		AI_UnequipWeapons(other);
		AI_UnequipWeapons(other);
	};
	...
	Info_ClearChoices(DIA_Harad_OrcSuccess);
	Info_AddChoice(DIA_Harad_OrcSuccess,"Proszê bardzo...",dia_harad_orcsuccess_yes);
};

func void dia_harad_orcsuccess_yes()
{
	if(Npc_HasReadiedWeapon(other))
	{
		AI_RemoveWeapon(other);
...
func void DIA_Harad_LEHRLING_OK()
{
	...
	if((other.guild == GIL_NONE) || (other.guild == GIL_NOV))
	{
		...
		B_LogEntry(TOPIC_OV,"Harad przyj¹³ mnie na swojego czeladnika. Droga do górnego miasta stoi przede mn¹ otworem.");
	};
	Log_CreateTopic(TOPIC_CityTeacher,LOG_NOTE);
	B_GivePlayerXP(XP_Lehrling);
	B_LogEntry(TOPIC_CityTeacher,"Harad mo¿e mnie nauczyæ rzemios³a kowalskiego. Mo¿e równie¿ zwiêkszyæ moj¹ si³ê.");
	AI_Teleport(Lothar,"NW_CITY_LOTHAR");
...
func void DIA_Harad_SellBlades_Info()
{
	...
	anzahl = Npc_HasItems(other,itmw_1h_sword_long_03);
	EquipWeap = Npc_GetEquippedMeleeWeapon(other);
	Npc_GetInvItem(other,itmw_1h_sword_long_03);
	OthWeap = Npc_GetReadiedWeapon(other);
	if((Hlp_IsItem(EquipWeap,itmw_1h_sword_long_03) || (Hlp_GetInstanceID(OthWeap) == Hlp_GetInstanceID(item))) && (anzahl == 1))
	{
	...
	else
	{
		...
		B_GiveInvItems(other,self,itmw_1h_sword_long_03,anzahl);
		AI_Output(self,other,"DIA_Harad_SellBlades_12_03");	//Dobrze. Oto twoja zap³ata.
		CreateInvItems(self,ItMi_Gold,VALUE_ITMW_1H_SWORD_LONG_03 * anzahl);
		B_GiveInvItems(self,other,ItMi_Gold,VALUE_ITMW_1H_SWORD_LONG_03 * anzahl);
		Npc_RemoveInvItems(self,itmw_1h_sword_long_03,Npc_HasItems(self,itmw_1h_sword_long_03));
		HARADCOMMON = TRUE;
	};
};
...
func void DIA_Harad_Erzklingen_Info()
{
	...
	else
	{
		...
		Info_AddChoice(DIA_Harad_Erzklingen,"Miecz dwurêczny (1500 sztuk z³ota)",DIA_Harad_Erzklingen_2h);
		Info_AddChoice(DIA_Harad_Erzklingen,"Miecz jednorêczny (1000 sztuk z³ota)",DIA_Harad_Erzklingen_1h);
	};
};
...
func void DIA_Harad_Erzklingen_2h()
{
	...
	if(Npc_HasItems(other,ItMi_Gold) >= Value_Blessed_2H_1)
	{
		...
		Npc_RemoveInvItems(self,ItMi_Gold,Value_Blessed_2H_1);
...
func void DIA_Harad_Erzklingen_1h()
{
	...
	if(Npc_HasItems(other,ItMi_Gold) >= Value_Blessed_1H_1)
	{
		...
		Npc_RemoveInvItems(self,ItMi_Gold,Value_Blessed_1H_1);
		B_Harad_HaveFunWithYourSword();
	}
	else
	{
...
{
	if((MIS_Bennet_InnosEyeRepairedSetting != LOG_SUCCESS) && !Npc_KnowsInfo(other,DIA_Bennet_RepairNecklace) && (SMITH_AMULET == TRUE))
	{
		return TRUE;
	};
};
...
{
	if(Npc_KnowsInfo(other,DIA_Harad_RepairNecklace) && !Npc_KnowsInfo(other,DIA_Bennet_RepairNecklace))
	{
		return TRUE;
	};
};
